...
emcc $FLAGS -std=c++11 -O3 -o dbow.o DBoW2/src/*.cpp;

emcc $FLAGS -std=c++11 -O3 -o openvslam.o src/openvslam/system.cc src/openvslam/tracking_module.cc src/openvslam/mapping_module.cc src/openvslam/global_optimization_module.cc src/openvslam/data/*.cc src/openvslam/module/*.cc src/openvslam/feature/*.cc src/openvslam/publish/*.cc src/openvslam/optimize/*.cc src/openvslam/match/*.cc src/openvslam/util/*.cc src/openvslam/optimize/g2o/*.cc src/openvslam/optimize/g2o/se3/*.cc src/openvslam/optimize/g2o/sim3/*.cc src/openvslam/config.cc src/openvslam/solve/*.cc src/openvslam/io/*.cc src/openvslam/initialize/*.cc src/openvslam/camera/*.cc src/socket_publisher/data_serializer2.cc

emcc $FLAGS -std=c++11 -O3 -o yaml.o yaml-cpp/src/*.cpp yaml-cpp/include/yaml-cpp/exceptions.cc yaml-cpp/include/yaml-cpp/node/impl.cc

emcc $FLAGS -O3 -o csparse.o g2o/EXTERNAL/csparse/*.c;
emcc $FLAGS -std=c++11 -O3 -o g2o.o g2o/g2o/core/*.cpp g2o/g2o/stuff/*.cpp g2o/g2o/solvers/csparse/*.cpp

emcc $FLAGS -std=c++11 -O3 -s TOTAL_MEMORY=400MB -o run_web.js csparse.o dbow.o openvslam.o yaml.o g2o.o opencv/build_wasm/lib/*.a opencv/build_wasm/3rdparty/lib/libzlib.a ./example/run_web.cc ./example/run_calibration.cc ./example/run_malloc.cc